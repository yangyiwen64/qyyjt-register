...
const { startBrowserRegistration, cancelRegistration, resetCancel } = require('./register_browser');
...
app.post('/api/register', (req, res) => {
  ...
  startBrowserRegistration(count,
    (data) => {
      ...
      if (data.account) {
        const acc = { id: generateId(), phone: data.account.phone, password: data.account.password, status: data.account.status, createTime: now(), remark: data.account.remark };
...
server.listen(PORT, () => {
  ...
  console.log(`[豪猪网] ${process.env.HAOZHU_USER || 'todayis0607'} / 项目 ${process.env.HAOZHU_PROJECT || '49827'}`);
});
