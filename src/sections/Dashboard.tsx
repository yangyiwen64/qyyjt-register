        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 -mt-0.5">
                  <p className="text-xs text-slate-400">真实浏览器自动化注册</p>
                  {currentUser && (
            ...
            <div className="flex items-center gap-1">
              {/* 后端连接状态 */}
              <div className="flex items-center gap-1.5 mr-3 px-2 py-1 rounded-md bg-slate-50">
                <div className={`w-2 h-2 rounded-full ${backendConnected ? 'bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.6)]' : 'bg-red-400'}`} />
                <span className="text-xs text-slate-500">{backendConnected ? '后端就绪' : '后端未连接'}</span>
              </div>
              {isAdmin && (
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
            ...
            <CardContent className="p-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-slate-700">
                      {isRegistering ? `真实注册进行中（${progress.total}个）` : '注册完成'}
                    </span>
                ...
                {progress.logs.length > 0 && (
                  <div className="border-t border-slate-100 pt-2 max-h-32 overflow-y-auto space-y-1">
                    {progress.logs.slice(0, 10).map((log, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-xs">
                        <span className="text-slate-400 tabular-nums">{log.time}</span>
                        <span className={log.type === 'success' ? 'text-emerald-600' : log.type === 'error' ? 'text-red-500' : log.type === 'warning' ? 'text-amber-600' : 'text-slate-600'}>{log.message}</span>
                      </div>
                    ))}
                  </div>
                )}
                {isRegistering && <p className="text-xs text-amber-600">正在调用豪猪网API+Playwright浏览器自动化，请勿刷新页面...</p>}
              </div>
            </CardContent>
          </Card>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3">
          {isRegistering ? (
            ...
            <Button onClick={() => setShowRegister(true)} className="bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white shadow-lg shadow-blue-500/20 h-10" disabled={!backendConnected}>
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-12 text-slate-400">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          {accounts.length === 0 && <p className="text-xs">点击「批量注册」开始真实注册</p>}
                        </div>
                      </TableCell>
                    </TableRow>
